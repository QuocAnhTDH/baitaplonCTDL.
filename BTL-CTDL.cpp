...
class QuanLi{
	...
	void nhapnhanvien()
	{
		...
		while(i<=n)
		{
			...
			NhanVien *p = head;
			//tao con tro p de duyet danh sach tu dau
			bool trung = false;
			// tao mot bien de danh dau ma nhan vien bi trung hay khong
			while(p != NULL){
				//lap cho den khi p = NULL
				if(p->MaNv == newNhanVien->MaNv){
					//neu manv p trung vs ma nhan vien moi
					trung = true;
					break;
					//ket thuc vong lap
				}
				p= p->next;
				//di chuyen p den nhan vien tiep theo
			}
			if(trung){
				//neu bien trung bang true
	...
	void Search()
	{
		...
		while(count != NULL)
		{
			...
			cout<<"KHONG TIM THAY NHAN VIEN TRONG DANH SACH \n";
				count= count->next;
			}
		}
	}
...
//	}
	void Luong10m()
	{
		long long int luong;
			...
			cin>>luong;
			cout<<"Nhan Vien Co Luong Lon Hon Hoac Bang"<<luong<<":\n";
			cout<<"MaNV|HovaTen|GioiTinh|NamSinh|NoiSinh|DiaChi|CCCD|ChucVu|LuongCoBan|PhuCap|ThucLinh\n";
			while(run != NULL)
			{
				
				if(run->ThucLinh >= luong)
				{
						cout<<run->MaNv<<"|"<<run->HoTenNv<<"|"<<run->GioiTinh<<"|"<<run->NamSinh<<"|"<<run->NoiSinh;
			    		cout<<"|"<<run->DiaChi<<"|"<<run->cccd<<"|"<<run->ChucVu<<"|"<<run->LuongCoBan<<"|"<<run->Phucap<<"|"<<run->ThucLinh<<"\n";
				}
